...
class CutMix(object):
    ...
    def __call__(self, train_batch):
        ...
        for i in range(len(train_batch)):
            ...
            image_batch_edit[i][0][:,px1:px2, py1:py2] = train_batch[rand_index[i]][0][:,px1:px2, py1:py2]
            lam_pic = 1 - ((px2 - px1) * (py2 - py1) / (image_batch_edit[i][0].shape[1] * image_batch_edit[i][0].shape[2]))
            mix_label = lam_pic * lam * np.eye(100)[train_batch[i][1]] + (1- lam_pic) * lam * np.eye(100)[train_batch[rand_index[i]][1]]
            img_batch_cutmix.append(image_batch_edit[i][0])
            labels.append(torch.from_numpy(mix_label))
