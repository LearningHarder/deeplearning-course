...
class FasterRCNNBase(nn.Module):
    ...
    @torch.jit.unused
    def eager_outputs(self, losses, detections,val_mode):
    ...
    def forward(self, images, targets=None,val_mode=False):
        ...
        proposals, proposal_losses = self.rpn(images, features, targets,val_mode)

        # 将rpn生成的数据以及标注target信息传入fast rcnn后半部分
        detections, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets,val_mode)

        # 对网络的预测结果进行后处理（主要将bboxes还原到原图像尺度上）
        detections = self.transform.postprocess(detections, images.image_sizes, original_image_sizes)
        #此时detections里的proposals已经缩放回
        ...
        if torch.jit.is_scripting():
            ...
            return self.eager_outputs(losses, detections,val_mode)
