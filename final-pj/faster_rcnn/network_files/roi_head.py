...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        #列表是1000维的 对应predict的一张图片里推荐的1000个proposals
        #proposals是一个list 里面只有一个元素(when pridict) 是一个torch size=[1000,4]
        
        # 根据proposal以及预测的回归参数计算出最终bbox坐标
        pred_boxes,proposalsv2 = self.box_coder.decode(box_regression, proposals)
        ...
        proposalsv2_list = proposalsv2.split(boxes_per_image, 0)
        ...
        # 遍历每张图像预测信息
        for boxes, scores, proposalsv2, image_shape in zip(pred_boxes_list, pred_scores_list, proposalsv2_list,image_shapes):
            # 裁剪预测的boxes信息，将越界的坐标调整到图片边界上
            boxes = box_ops.clip_boxes_to_image(boxes, image_shape)
            
            #裁剪边缘的proposals信息 
            proposalsv2=box_ops.clip_boxes_to_image(proposalsv2, image_shape)
            ...
            # 移除索引为0的所有信息（0代表背景）
            boxes = boxes[:, 1:]#1000*21*4=>1000*20*4
            scores = scores[:, 1:]
            labels = labels[:, 1:]
            ...
            if proposalsv2.size()[0]!=0:
              all_proposalsv2.append(proposalsv2[:,0])
            all_proposalsv2.append(proposalsv2)
            all_boxes.append(boxes)
            all_scores.append(scores)
            all_labels.append(labels)

        return all_proposalsv2,all_boxes, all_scores, all_labels
    ...
    def forward(self,
        ...
        if self.training or val_mode:
            ...
            proposals, boxes, scores, labels = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
