...
def create_model(num_classes, load_pretrain_weights=False):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="./backbone/mask_rcnn_weights_res50_backbone.pth",
...
def main(args):
    ...
    if os.path.exists('./tb_res_vis') is False:
        os.makedirs('./tb_res_vis')
    
    tb_writer=SummaryWriter(log_dir='./tb_res_vis')

    for epoch in range(args.start_epoch, args.epochs):
        # tloss = utils.test_one_epoch(model, val_data_set_loader,
        ...
        tb_writer.add_scalar('test_loss',tloss.item(),epoch)
        tb_writer.add_scalar('test_mAP',coco_info[1],epoch)
